...
| 🔄 **Restauração automática de mesa** | Quando o cliente faz login, se houver pedido em aberto no banco, o app busca a mesa associada e restaura `TABLE_NUMBER`/`MESA_ID` no AsyncStorage automaticamente — cliente cai direto no fluxo de pedidos sem precisar escanear o QR de novo. Caso típico: cliente fechou o app no meio da refeição e voltou pra finalizar a conta. |
